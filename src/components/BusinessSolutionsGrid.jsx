...
export default function SmartSolutions() {
  ...
  <p className="text-black-600 font-semibold mb-10 max-w-2xl mx-auto">
    Our team of experts is dedicated to crafting customized business strategies that
    align with your vision and goals. We specialize in:
  </p>
  ...
  <div className="max-w-7xl mx-auto ">
    <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3 justify-items-center">
      ...
      <div
      key={idx}
      className="relative w-[380px] h-[380px] rounded-xl overflow-hidden shadow-md group"
      style={{
    ...
    >
      ...
      {/* Text Content */}
      <div className="absolute inset-0 z-30 flex flex-col justify-end group-hover:justify-center items-start group-hover:items-center px-6 transition-all duration-300 text-left group-hover:text-center">
        <div className="w-full max-w-[260px] mt-auto group-hover:mt-0 transition-all duration-300" >
          <h3 className="text-[30px] font-semibold leading-snug text-white group-hover:text-[22px] group-hover:mb-6">
            {service.title}
          </h3>
      ...
      <div className="absolute bottom-4 right-4 z-30 opacity-100 group-hover:opacity-0 transition-opacity duration-300">
        <img src={service.icon} alt={`${service.title} icon`} className="w-16 h-12" />
      </div>
    </div>     
      ))}
    </div>
